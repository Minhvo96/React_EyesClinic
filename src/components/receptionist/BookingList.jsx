...
export default function BookingList() {
  ...
  const [timesMorning, setTimeMorning] = useState([
    "08:00",
    "08:30",
    "09:00",
    "09:30",
  ...
  const getAllBookingList = async () => {
    ...
    const bookingsPending =
      await bookingService.getBookingByStatusPendingAndDate(newBooking);
  ...
  const setTimesFreeByDate = async (dateBooking) => {
    ...
    const bookingsWaiting =
      await bookingService.getBookingByStatusWaitingAndDate(newBooking);
    ...
    if (new Date(dateBooking) > currentDate) {
      const listTimeFreeBooking = times.filter(
        (item) => !listTimeBooked.includes(item)
      );
      ...
      const listTimeFreeBooking = times.filter(
        (item) =>
          ...
          item.localeCompare(formattedTime) > 0
      );
      setTimesFreeBooking(listTimeFreeBooking);
    }
  };
  ...
  const getEyeCategoryById = async (e) => {
    const eyeCategory = await eyeCategoriesService.getEyeCategoryById(
      e.target.value
    );
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setBookingList((booking) =>
          booking.filter((booking) => booking.id !== bookingId)
        );
      }
    });
  };
  ...
  const handleChangeStatusBooking = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        setBookingList((booking) =>
          booking.filter((booking) => booking.id !== id)
        );
      }
    });
  };
  ...
  const handleChangeListBookingByTime = (time) => {
    ...
    if (time === "all") {
      ...
      setBookingListByTime(
        bookingList.filter((item) => timesMorning.includes(item.timeBooking))
      );
    } else if (time === "afternoon") {
      setBookingListByTime(
        bookingList.filter((item) => timesAfternoon.includes(item.timeBooking))
      );
    }
  };
  ...
  const currentAppointments = bookingListByTime
    .sort((a, b) => {
  ...
  return (
    <>
      <div className="container-fluid">
        ...
        <div
          className="col-lg-12 d-flex align-items-around"
          style={{ padding: 0 }}
        >
                ...
                <div className="d-flex align-items-center">
                  ...
                  <div className="col-7 ">
                    <input
                      ...
                      onChange={handleChangeListByDate}
                      min={defaultDate}
                    />
                  </div>
                </div>
                <div className="d-flex align-items-center gap-4">
                  <div>
                    <button
                      ...
                      style={{
                        backgroundColor:
                          selectedTime === "all" ? "#007bff" : "initial",
                  ...
                  <div>
                    <button
                      ...
                      style={{
                        backgroundColor:
                          selectedTime === "morning" ? "#007bff" : "initial",
                  ...
                  <div>
                    <button
                      ...
                      style={{
                        backgroundColor:
                          selectedTime === "afternoon" ? "#007bff" : "initial",
                        color:
                          selectedTime === "afternoon" ? "#fff" : "initial",
                      }}
                    >
                      Chiều
                    </button>
              ...
              {loading ? (
                ...
                <>
                  <table className="table">
                    ...
                    <tbody>
                      {currentAppointments.map((booking, index) => {
                        ...
                        return (
                          <tr key={booking.id}>
                            ...
                            <td className="text-center">
                              {booking.eyeCategory.nameCategory}
                            </td>
                            <td className="text-center">
                              <button
                              ...
                              <button
                                ...
                                onClick={() =>
                                  handleChangeStatusBooking(booking.id)
                                }
                              >
                                Xác nhận khám
                              </button>
                ...
                <div className="m-4">
                  ...
                  >
                    <div>
                      <i
                        className="fa-regular fa-calendar-xmark text-danger"
                        style={{ fontSize: "124px" }}
                      ></i>
                    </div>
                    <span className="fw-semibold" style={{ fontSize: "32px" }}>
                      Danh sách hôm nay đang trống!
                    </span>
                  </div>
                </div>
              )}
                  ...
                  <div className="row mb-3">
                    ...
                    <div className="col-md-6">
                      ...
                      <select
                        ...
                        id="categorySelect"
                        className="form-control"
                        onChange={getEyeCategoryById}
                      >
                        <option value={booking.eyeCategory.id}>
                          {booking.eyeCategory.nameCategory}
                        </option>
                        ...
                        {timesFreeBooking.map((time, index) => {
                          return (
                            <option
                              ...
                              value={time}
                              selected={
                                time == booking.timeBooking ? true : false
                              }
                            >
                              {time}
                            </option>
